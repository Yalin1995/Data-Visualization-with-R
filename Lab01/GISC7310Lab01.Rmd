...
skew3 <- e1071::skewness(car::bcPower(CPS1985$wage, lambda=-1.5))
...
```{r}
f <- function(lambda){
  A <- integrate(function(x) {exp(-x^lambda)}, 0, Inf)
  fnE <- function(x) {x*exp(-x^lambda)/A$value}
  E <- integrate(fnE, 0, Inf)
  fnV <- function(x) {((x-E$value)^2)*exp(-x^lambda)/A$value}
  Var <- integrate(fnV, 0, Inf)
